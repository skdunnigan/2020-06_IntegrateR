...
fish_long_sub %>% 
  ...
  ggplot() +
  geom_density(aes(x = date, y = mean, color = site, fill = site), alpha = 0.3, stat = "identity") +
...
##  4a. Using the subsetted data frame, calculate the total number caught of each species in this data set.  

```{r}
fish_long_sub %>% 
  group_by(species) %>% 
  summarize(total_caught = sum(count))
```


##  4b. Using the subsetted data frame, calculate the total number caught of each species in this data set, *by site*.  

Save this as a data frame called `fish_totals` and print it.  

```{r}
fish_totals <- fish_long_sub %>% 
