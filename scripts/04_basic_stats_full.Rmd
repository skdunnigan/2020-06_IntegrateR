...
ggplot(wq) +
    geom_boxplot(aes(x = state, y = temp, fill = state)) +
    theme(legend.position = "none") +
    labs(title = "Is Temperature different between states?")
```
...
```{r}
ggplot(wq) +
    geom_boxplot(aes(x = state, y = ph, fill = state)) +
    theme(legend.position = "none") +
    labs(title = "Is pH different between states?")

ph_lm <- lm(ph ~ state, data = wq)
ph_aov <- anova(ph_lm)
ph_aov
tidy(ph_aov)
glance(ph_aov) # hm, deprecated for anova tables
glance(ph_lm)  # still useful for lm objects though
```
...
```{r}
ggplot(wq, aes(x = sal, y = ph)) +
  geom_point(aes(color = state), size = 3) +
  labs(title = "Is pH related to salinity?") +
  geom_smooth(method = "lm")

sal_ph <- lm(ph ~ sal, data = wq)
summary(sal_ph)
```
